// Ss5-client forwards local SOCKS5 connections through mutually authenticated TLS.
package main
...
// client forwards local SOCKS5 traffic to a selected upstream over mutual TLS.
type client struct {
	*ss5.Service
...
// upstreamEndpoint retains the hostname so each dial can resolve it again.
type upstreamEndpoint struct {
	auth      *ss5.Credentials
	negotiate bool
	addrStr   string
	tlsConfig *tls.Config
}

// Config holds the client settings read from the JSON configuration file.
type Config struct {
	// ServerAuth maps upstream addresses to SOCKS5 credentials. When nonempty,
	// the client accepts local SOCKS5 without authentication and authenticates
	// upstream connections itself. Unlisted upstreams use no SOCKS5 authentication.
	// An empty map leaves SOCKS5 authentication to the local caller.
	ServerAuth map[string]ss5.Credentials `json:"server_auth,omitempty"`
	ServerAddr []string                   `json:"server_addr"` // Upstream host:port addresses.
	ClientPEM  string                     `json:"client_pem"`  // Client certificate file.
	ClientKey  string                     `json:"client_key"`  // Client private key file.
	ServerPEM  string                     `json:"server_pem"`  // Trusted server CA file.
	ListenAddr string                     `json:"listen_addr"` // Local SOCKS5 listen address.
}

// serverDialer is shared by concurrent upstream connection attempts.
var serverDialer = &net.Dialer{
	Timeout:   30 * time.Second,
	KeepAlive: 30 * time.Second,
}

// NewClient configures the listener, upstreams and TLS credentials.
func NewClient(listen string, serverAddrs []string, clientPEM, clientKey, serverPEM string, authMap map[string]ss5.Credentials) (*client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("resolve listen address %q: %w", listen, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("read server CA %q: %w", serverPEM, err)
	}
	serverCertPool := x509.NewCertPool()
	if !serverCertPool.AppendCertsFromPEM(serverCertBytes) {
		return nil, errors.New("failed to parse PEM-encoded server certificates")
	}

	// Each endpoint gets its own ServerName while sharing the session cache.
	baseTLS := &tls.Config{
	...
	for endpoint, auth := range authMap {
		found := false
		for _, configured := range serverAddrs {
		...
		if !found {
			return nil, fmt.Errorf("authentication configured for unknown upstream %q", endpoint)
		}
		if err := auth.Validate(); err != nil {
			return nil, fmt.Errorf("invalid authentication for upstream %q: %w", endpoint, err)
		}
	}
	...
	for _, serverAddr := range serverAddrs {
		...
		if value, ok := authMap[serverAddr]; ok {
			auth = &value
		}
	...
	if len(upstreams) == 0 {
		return nil, errors.New("no server addresses provided")
	}
...
// Listen accepts local SOCKS5 connections until a shutdown signal is received.
// It stops probes and the connection pool before returning.
func (c *client) Listen() error {
	defer c.cancel()

	for _, upstream := range c.upstreams {
		log.Printf("The configured server address is %s.", upstream.addrStr)
	}

	stableIndex := int(c.stableIndex.Load())
	if stableIndex >= len(c.upstreams) {
		stableIndex = 0
		c.stableIndex.Store(0)
	}
	stable := c.upstreams[stableIndex]
	log.Printf("Using the default server address: %s.", stable.addrStr)
	...
	defer stopSignals()
	return ss5.Serve(stop, listener, 1024, func(ctx context.Context, conn net.Conn) {
		c.handleConn(ctx, conn.(*net.TCPConn))
	})
}

// handleConn configures and forwards one local SOCKS5 connection.
func (c *client) handleConn(ctx context.Context, userConn *net.TCPConn) {
	ss5.ConfigureTCPConn(userConn)
	c.connectServer(ctx, userConn)
}

// connectServer obtains an upstream connection and relays traffic to userConn.
func (c *client) connectServer(ctx context.Context, userConn *net.TCPConn) {
	if c.negotiate {
		_ = userConn.SetDeadline(time.Now().Add(30 * time.Second))
		if err := ss5.NegotiateServer(userConn, nil); err != nil {
	...
	var err error
	if c.pool != nil {
		serverConn, err = c.connectPrepared(ctx, userConn)
	} else {
		serverConn, _, err = c.acquireServer(ctx, false)
	}
	if err != nil {
	...
	if err := ss5.RelayClient(userConn, serverConn); err != nil {
...
func dialUpstream(ctx context.Context, upstream upstreamEndpoint) (net.Conn, error) {
	...
	stop := context.AfterFunc(ctx, func() { ss5.CloseConnection(conn) })
	defer stop()

	if upstream.negotiate {
		deadline, _ := ctx.Deadline()
		_ = conn.SetDeadline(deadline)
		if err := ss5.NegotiateClient(conn, upstream.auth); err != nil {
...
// connectPrepared exchanges one SOCKS5 request and reply before relaying data.
// It retries a failed pooled connection only before application data is forwarded.
// A valid SOCKS5 failure reply is returned unchanged, never retried.
func (c *client) connectPrepared(ctx context.Context, userConn net.Conn) (net.Conn, error) {
	...
	if request[1] != ss5.CmdConnect && request[1] != ss5.CmdUDPAssociate {
		ss5.SendSOCKS5Reply(userConn, 0x07)
		return nil, errors.New("unsupported SOCKS5 command")
	}

	conn, pooled, err := c.acquireServer(ctx, true)
	if err != nil {
		ss5.SendSOCKS5Reply(userConn, 0x01)
		return nil, err
	}

	for attempt := 0; ; attempt++ {
		current := conn
		stop := context.AfterFunc(ctx, func() { ss5.CloseConnection(current) })
		_ = conn.SetDeadline(time.Now().Add(30 * time.Second))

		err = ss5.WriteAll(conn, request)
		var reply []byte
		if err == nil {
		...
		if err == nil {
			...
			if err := ss5.WriteAll(userConn, reply); err != nil {
				ss5.CloseConnection(conn)
				return nil, err
			}
			if reply[1] != 0 {
...
// readSOCKSFrame reads one SOCKS5 request or reply, leaving application data unread.
func readSOCKSFrame(r io.Reader) ([]byte, error) {
	frame := make([]byte, 4, 262)
	if _, err := io.ReadFull(r, frame); err != nil {
		return nil, err
	}
	if frame[0] != ss5.SocksVersion || frame[2] != 0 {
		return nil, errors.New("invalid SOCKS5 header")
	}
	...
	case ss5.AtypDomain:
		...
		n = int(frame[4])
		if n == 0 {
			return nil, errors.New("empty SOCKS5 domain")
		}
	default:
...
// enablePreconnect enables the pool only when the client owns authentication.
...
// preconnectPool holds authenticated connections that have not received a SOCKS5
// request. Active tunnels belong to their callers and are never returned.
...
// Requests reactivate refilling after an idle period.
func (p *preconnectPool) take(index int) net.Conn {
	...
	for len(p.items) > 0 {
		item := p.items[0]
		// Keep the backing array available for refills and release the last reference.
		copy(p.items, p.items[1:])
		last := len(p.items) - 1
		p.items[last] = idleConnection{}
		p.items = p.items[:last]
		if !time.Now().Before(item.expires) {
			ss5.CloseConnection(item.conn)
			continue
		}
		return item.conn
	}
...
// selectUpstream drains unused connections when the selected upstream changes.
// Active tunnels stay open; results from earlier dial generations are discarded.
...
// run refills the pool while it is in use and drains pending results on shutdown.
func (p *preconnectPool) run() {
	...
	defer func() {
		for pending > 0 {
			...
			pending--
			if result.conn != nil {
				ss5.CloseConnection(result.conn)
			}
		}
	}()
...
// pruneExpiredLocked closes expired idle connections. The caller must hold p.mu.
...
// acceptResult keeps a current, unexpired connection or closes it.
// Failed dials share a backoff window.
...
// resetDialsLocked cancels pending dials and advances the generation so late
// results are discarded. The caller must hold p.mu.
...
// close cancels dials, closes unused connections and waits for run to finish.
// The caller must have started run.
...
// upstreamSelector serializes probe selection and foreground failover.
...
// plan returns the current upstream first, followed by fallbacks, and the generation.
func (s *upstreamSelector) plan() ([]int, uint64) {
	s.mu.Lock()
	defer s.mu.Unlock()

	current := int(s.client.stableIndex.Load())
	order := make([]int, 1, len(s.nodes))
	order[0] = current
	for i := range s.nodes {
		if i != current {
...
// probeUpstream checks TLS and SOCKS5 readiness on a separate connection.
// In passthrough mode it checks method selection only, because credentials
...
func probeUpstream(parent context.Context, endpoint upstreamEndpoint) error {
	...
	defer ss5.CloseConnection(conn)
	if endpoint.negotiate {
		return nil
	}
	...
	deadline, _ := ctx.Deadline()
	_ = conn.SetDeadline(deadline)
	if err := ss5.WriteAll(conn, []byte{ss5.SocksVersion, 2, 0, 2}); err != nil {
		return err
	}
	...
	if reply[0] != ss5.SocksVersion || (reply[1] != 0 && reply[1] != 2) {
...
func main() {
	...
	c, err := NewClient(config.ListenAddr, config.ServerAddr, config.ClientPEM, config.ClientKey, config.ServerPEM, config.ServerAuth)
	if err != nil {
